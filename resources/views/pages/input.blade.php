<x-admin-layout>
    ...
    <nav class="relative flex flex-wrap items-center justify-between px-0 py-2 mx-6 transition-all shadow-none duration-250 ease-soft-in rounded-2xl" navbar-main navbar-scroll="true">
        <div class="flex items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit">
          <nav>
            ...
            <ol class="flex flex-wrap pt-1 mr-12 bg-transparent rounded-lg sm:mr-16">
              ...
              <li class="text-sm pl-2 capitalize leading-normal text-slate-700 before:float-left before:pr-2 before:text-gray-600 before:content-['/']" aria-current="page">Input Data</li>
            </ol>
            <h6 class="mb-0 font-bold capitalize">Input Data</h6>
          </nav>
      ...
      <div class="md:grid md:grid-cols-3 md:gap-6">
        <div class="px-5 py-5 rounded mt-5 shadow md:col-span-2 md:mt-0">
            <form action="{{route('student-data.store')}}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="name" id="desc" placeholder="Input Name"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-96 block p-2.5">
                    @if ($errors->has('name'))
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="student_id" id="student_id" placeholder="Input ID"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-96 p-2.50">
                    @if ($errors->has('student_id'))
                        <span class="text-red-500 text-sm">{{ $errors->first('student_id') }}</span>
                    @endif
                </div>
                
                <div class="mb-6">
                    <label for="gpa" class="block mb-2 text-sm font-medium text-gray-900">Student GPA</label>
                    <input type="float" name="gpa" id="gpa" placeholder="Input GPA"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-96 block p-2.5">
                    @if ($errors->has('gpa'))
                        <span class="text-red-500 text-sm">{{ $errors->first('gpa') }}</span>
                    @endif
                </div>
                <button type="submit" class="text-white bg-teal-500 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 focus:outline-none">Save</button>
            </form>
        </div>
    </div> 
</x-admin-layout>